...
/- removed value specification -/
 ...
 partial def  boolListFrombitSeqAux  {a : Type}  (n : Nat) (s : a) (bl : List a)  : List a := 
  if  n  ==   0 then  []  else 
...
def  bitSeqTestBit  (b : bitSequence) (pos : Nat)  : Option (Bool) := match  (b,pos) with |  (( BitSeq  len  s  bl),  pos)  =>  match  len with  |  none =>  if  natLtb  pos  (List.length  bl) then  List.get?  bl  pos  else  some  s |  some l =>  if  ( natGteb pos  l) then  none  else                  if  ((pos  ==  (l  -   1))  ||  natGteb  pos  (List.length  bl)) then  some  s  else                  List.get?  bl  pos    
/- removed value specification -/

def  bitSeqSetBit  (b : bitSequence) (pos : Nat) (v : Bool)  : bitSequence := match  (b,pos,v) with |  (( BitSeq  len  s  bl),  pos,  v)  =>  let  bl'  := if  ( natLtb pos  (List.length  bl)) then  bl  else  bl  ++  List.replicate  pos  s 
   let  bl''  := List.update  bl'  pos  v 
   let  bs'  := BitSeq  len  s  bl'' 
   cleanBitSeq  bs' 
/- removed value specification -/

def  resizeBitSeq  (new_len : Option (Nat)) (bs : bitSequence)  : bitSequence := 
  match  cleanBitSeq  bs with | (  BitSeq  len  s  bl) =>    let  shorten_opt  := match  (new_len, len) with  |  (none,  _) =>  none |  (some l1,  none) =>  some  l1 |  (some l1,  some l2) =>  if  ( natLtb l1  l2) then  some  l1  else  none    
   match  shorten_opt with  |  none =>  BitSeq  new_len  s  bl |  some l1 =>  (         let  bl'  := List.take  l1  (bl  ++  [s]) 
         match  dest_init  bl' with  |  none =>  (BitSeq  len  s  bl) |  some (bl'',  s') =>  cleanBitSeq  (BitSeq  new_len  s'  bl'') 	)    
/- removed value specification -/
...
/- 
 partial def  bitSeqBinopAux  (binop : Bool → Bool → Bool) (s1 : Bool) (bl1 : List (Bool)) (s2 : Bool) (bl2 : List (Bool))  : List (Bool) := 
...
def  bitSeqBinop  (binop : Bool → Bool → Bool) (bs1 : bitSequence) (bs2 : bitSequence)  : bitSequence :=  (
  match  cleanBitSeq  bs1 with | (  BitSeq  len1  s1  bl1) =>    match  cleanBitSeq  bs2 with | (  BitSeq  len2  s2  bl2) =>     let  len  := match  (len1, len2) with  |  (some l1,  some l2) =>  some  (natMax  l1  l2) |  _ =>  none    
   let  s  := binop  s1  s2 
...
/- removed value specification -/
 ...
 partial def  integerFromBoolListAux   (acc  : Int)  (bl  : List  Bool)  : Int :=  
...
def  bitSeqArithBinOp  (binop : Int → Int → Int) (bs1 : bitSequence) (bs2 : bitSequence)  : bitSequence := 
  match  bs1 with | (  BitSeq  len1  _  _) =>    match  bs2 with | (  BitSeq  len2  _  _) =>    let  len  := match  (len1, len2) with  |  (some l1,  some l2) =>  some  (natMax  l1  l2) |  _ =>  none    
...
instance   : Ord bitSequence where
...
def  natFromBitSeq  (bs : bitSequence)  : Nat :=    (naturalFromBitSeq  (resizeBitSeq  (some (  31))  bs))
/- removed value specification -/
 
def  bitSeqFromNat  (i : Nat)  : bitSequence :=  bitSeqFromNatural  (some (  31))  (  i)
/- removed value specification -/
